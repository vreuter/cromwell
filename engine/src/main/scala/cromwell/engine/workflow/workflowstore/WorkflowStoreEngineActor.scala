...
final case class WorkflowStoreEngineActor private(store: WorkflowStore,
  ...
  private def newWorkflowMessage(maxWorkflows: Int): Future[WorkflowStoreEngineActorResponse] = {
    def fetchStartableWorkflowsIfNeeded = {
      if (maxWorkflows > 0) {
        workflowStoreWriter.fetchStartableWorkflows(maxWorkflows, workflowHeartbeatConfig.cromwellId, workflowHeartbeatConfig.ttl)
      } else {
...
object WorkflowStoreEngineActor {
  def props(
    ...
    Props(WorkflowStoreEngineActor(workflowStore, workflowStoreWriter, serviceRegistryActor, abortAllJobsOnTerminate, workflowHeartbeatConfig)).withDispatcher(EngineDispatcher)
  }
