...
import cromwell.engine.workflow.workflowstore.CoordinatedWorkflowStoreAccessActor._
import mouse.all._
...
/**
  ...
  */
class CoordinatedWorkflowStoreAccessActor(workflowStore: WorkflowStore) extends Actor {
...
object CoordinatedWorkflowStoreAccessActor {
  ...
  def props(workflowStore: WorkflowStore): Props = Props(new CoordinatedWorkflowStoreAccessActor(workflowStore)).withDispatcher(Dispatcher.IoDispatcher)
}
