    ...
    case (oldState, terminalState: WorkflowActorTerminalState) =>
      ...
      if (WorkflowLogger.isEnabled) {
        /*
          ...
          */
        def bruteForceWorkflowOptions: WorkflowOptions = WorkflowOptions.fromJsonString(workflowToStart.sources.workflowOptionsJson).getOrElse(WorkflowOptions.fromJsonString("{}").get)
